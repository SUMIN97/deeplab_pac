...
class ExtRandomHorizontalFlip(object):
    ...
    def __call__(self, img, lbl, nghbr):
        """
        Args:
        ...
        if random.random() < self.p:
            return F.hflip(img), F.hflip(lbl), F.hflip(nghbr)
        return img, lbl, nghbr
...
class ExtCenterCrop(object):
    ...
    def __call__(self, img, lbl, nghbr):
        ...
        """
        return F.center_crop(img, self.size), F.center_crop(lbl, self.size), F.center_crop(nghbr, self.size)
...
class ExtRandomScale(object):
    ...
    def __call__(self, img, lbl, nghbr):
        ...
        return F.resize(img, target_size, self.interpolation), F.resize(lbl, target_size, Image.NEAREST), \
               F.resize(nghbr, target_size, Image.NEAREST)
...
class ExtToTensor(object):
    ...
    def __call__(self, pic, lbl, nghbr):
        """
        ...
        """
        if self.normalize:
            return F.to_tensor(pic), torch.from_numpy( np.array( lbl, dtype=self.target_type) ), nghbr
        else:
...
class ExtNormalize(object):
    ...
    def __call__(self, tensor, lbl, nghbr):
        """
        Args:
        ...
        """
        return F.normalize(tensor, self.mean, self.std), lbl, nghbr
...
class ExtRandomCrop(object):
    ...
    def __call__(self, img, lbl, nghbr):
        ...
        if self.padding > 0:
            ...
            nghbr = F.pad(nghbr, self.padding)

        # pad the width if needed
        if self.pad_if_needed and img.size[0] < self.size[1]:
            img = F.pad(img, padding=int((1 + self.size[1] - img.size[0]) / 2))
            lbl = F.pad(lbl, padding=int((1 + self.size[1] - lbl.size[0]) / 2))
            nghbr = F.pad(nghbr, padding=int((1 + self.size[1] - nghbr.size[0]) / 2))

        # pad the height if needed
        if self.pad_if_needed and img.size[1] < self.size[0]:
            img = F.pad(img, padding=int((1 + self.size[0] - img.size[1]) / 2))
            lbl = F.pad(lbl, padding=int((1 + self.size[0] - lbl.size[1]) / 2))
            nghbr = F.pad(nghbr, padding=int((1 + self.size[0] - nghbr.size[1]) / 2))

        i, j, h, w = self.get_params(img, self.size)

        return F.crop(img, i, j, h, w), F.crop(lbl, i, j, h, w), F.crop(nghbr, i, j, h, w)
...
class ExtResize(object):
    ...
    def __call__(self, img, lbl, nghbr):
        ...
        Returns:
            PIL Image: Rescaled image.
        """
        return F.resize(img, self.size, self.interpolation), F.resize(lbl, self.size, Image.NEAREST), F.resize(nghbr, self.size, Image.NEAREST)
...
class ExtColorJitter(object):
    ...
    def __call__(self, img, lbl, nghbr):
        ...
        return transform(img), lbl, nghbr
